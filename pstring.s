...
swapCase:
    ...
    jmp .before_while_2
.while_2:
    movb (%rdi, %r8, 1), %r9b  #r9b <- thr current character
    cmpb $97, %r9b        # if char > a
    jge .more_a
    cmpb $65, %r9b        # id char > A
    jge .more_A
    jmp .before_while_2
.more_A:
    cmpb $90, %r9b       # if char < Z
    jl .more_A_less_Z
    jmp .before_while_2
.before_while_2:
    addb $1, %r8b     #counter++
...
.more_A_less_Z:
    leaq (%rdi, %r8, 1), %r10
    addb $32, (%r10)         #replace the char
    jmp .before_while_2
.more_a:
    cmpb $122, %r9b       # if char < z
    jl .more_a_less_z
    jmp .before_while_2
.more_a_less_z:
    leaq (%rdi, %r8, 1), %r10
    subb $32, (%r10)       #replace the char
    jmp .before_while_2
.end_while_2:
    pop %r10
    movq %rdi, %rax
...
.before_while_4:
    addb $1, %r8b      #counter++
    cmpb %r8b, %cl     # while (counter < len)
    jge .while_4
    jmp .equals
.while_4:
    movb (%rdi, %r8, 1), %r9b
    movb (%rsi, %r8, 1), %r10b
    cmpb %r9b, %r10b     #compare the char
    jg .pstr1_smaller
    cmpb %r10b, %r9b
    jg .pstr2_smaller
    jmp .before_while_4  #equls
.pstr1_smaller:
    movq $-1, %rax
    jmp .finish
.pstr2_smaller:
